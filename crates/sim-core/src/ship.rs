...
impl Ship {
    ...
    /// FlightPlan must be in order, and not overlap. Enforcement is getting deferred either to
    /// UI layer or Planner methods
    /// max_accel enforcement is being deferred to UI layer - pushing "more" at the max
    /// results in the max
    pub fn fly(&mut self, plan: &FlightPlan) -> StateVector {
        for m in plan.maneuvers() {
            ...
            if start > self.clock.now() {
                let duration = (start - self.clock.now()).to_seconds();
